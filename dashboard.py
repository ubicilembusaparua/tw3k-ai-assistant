"""Streamlit dashboard for the captured TW3K assistant LLM metrics."""
...
from db_query import get_conversation_metrics
...
def load_metrics(limit: int = 5000):
...
st.caption("Operational and user-feedback metrics captured from the PostgreSQL conversation log.")
...
if not records:
    st.info("No captured conversations yet. Ask a question in the main assistant to populate this dashboard.")
    st.stop()
...
with quality_right:
    ...
    st.dataframe(judge_rows, hide_index=True, use_container_width=True)
